...
class Solver():
	...
	def plot_cycles(self, cycles):
		...
		plt.plot(time, rs, label='CCA Sending Rate $r(t)$', color="k")
		plt.axhline(dqrate, label='Bottleneck Dequeue Rate $r$', linestyle='--', color="k")
		plt.fill_between(time, rs, dqrate, where=(rs < dqrate), label="Deficit", color="g", alpha=0.2, hatch="//", interpolate=True)
		plt.fill_between(time, dqrate, rs, where=(rs > dqrate), label="Overshoot", color="r", alpha=0.2, hatch="\\\\", interpolate=True)
		plt.xlabel('Time ($t$)')
		plt.ylabel('Mbps')
		plt.legend()
		plt.grid(True)
		...
		plt.fill_between(time, 0, qsize, label='Phantom Queue Occupancy $Q(t)$', color='gray', alpha=0.4, hatch="\\\\\\")
		plt.xlabel('Time ($t$)')
